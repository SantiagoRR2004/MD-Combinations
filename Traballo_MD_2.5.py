...
def combinaciones_sin_repeticiones(n, r):
    ...
    if n > 900 or r > 900:
        return int(
            factorial_iterativa(n)
            / (factorial_iterativa(n - r) * factorial_iterativa(r))
        )
...
def algoritmo(opciones, huecos, opcionesImportantes):
    ...
    for i in range(huecos + 1):
        ...
        posibilidadesNegativas = combinaciones_con_repeticion(
            opciones - opcionesImportantes, huecos - i
        )
    ...
    # Calculamos las posibilidades sin repeticiones
        ...
        combinatoriaSinR = []
        for i in range(huecos + 1):
            posibilidadesPositivas = combinaciones_sin_repeticiones(
                opcionesImportantes, i
            )
            posibilidadesNegativas = combinaciones_sin_repeticiones(
                opciones - opcionesImportantes, huecos - i
            )
...
if numPreguntas > temasTotales:
    ...
    combinatoriaSinR, combinatoriaConR = algoritmo(
        temasTotales, numPreguntas, temasEstudiados
    )
...
if numPreguntas > temasTotales:
    print(
        "No se puede calcular los porcentajes sin repetir temas porque hay más preguntas que temas."
    )
else:
    print(
    ...
    for i in range(len(combinatoriaSinR) - 1):
        print(
            f"La probabilidad de acertar más de {i} preguntas es de {sum(combinatoriaSinR[i+1:])/totalSinR*100:.2f}%"
        )
...
for i in range(len(combinatoriaConR) - 1):
    print(
        f"La probabilidad de acertar más de {i} preguntas es de {sum(combinatoriaConR[i+1:])/totalConR*100:.2f}%"
    )

plt.figure(figsize=(17, 9))
plt.title(
    f"{temasTotales} temas, {numPreguntas} preguntas, {temasEstudiados} estudiados",
    fontsize=22,
)
plt.grid(zorder=-10)
...
if numPreguntas <= temasTotales:
    ...
    plt.scatter(
        [x for x in range(numPreguntas + 1)],
        [x / totalSinR * 100 for x in combinatoriaSinR],
        s=50,
        c="r",
        zorder=10,
        marker="o",
    ...
    plt.scatter(
        [x for x in range(numPreguntas)],
        [
            sum(combinatoriaSinR[x + 1 :]) / totalSinR * 100
            for x in range(len(combinatoriaSinR) - 1)
        ],
        s=50,
        c="#FF00FF",
        zorder=10,
...
plt.scatter(
    [x for x in range(numPreguntas + 1)],
    [x / totalConR * 100 for x in combinatoriaConR],
    s=50,
    c="b",
    zorder=10,
    marker="o",
...
plt.scatter(
    [x for x in range(numPreguntas)],
    [
        sum(combinatoriaConR[x + 1 :]) / totalConR * 100
        for x in range(len(combinatoriaConR) - 1)
    ],
    s=50,
    c="#00FCFF",
    zorder=10,
